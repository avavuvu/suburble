<script lang="ts">
    ...
    import statusManager from "@lib/statusManager.svelte.ts";
    ...
    let isSelectableSuburbs = $derived(
        inputValue.length > 2 && potentialSuburbs.length > 0,
    );
    ...
    const inputChanged = async () => {
        ...
        if (potentialSuburbs.length < 8) {
            const cacheSuburbs = potentialSuburbs.map((suburb) =>
                suburbCache.get(suburb),
            );
        ...
        if (potentialSuburbs.length === 0) {
            const correctionAttempt = suburbNameSearcher.search(
                normalizedInput,
                {
                    threshold: 0.7,
                },
...
<div class="relative bg-white">
        ...
        >
            {#each potentialSuburbs as suburb}
                <button
                    ...
                    class:only-option={(potentialSuburbs.length === 1 ||
                        inputValue === suburb.toLowerCase()) &&
                        !isSuburbButtonFocused}
                    class="
                        suburb-button
    ...
    <form
        class="grid grid-cols-2 grid-rows-1 border border-black w-full rounded overflow-clip
    ...
    >
        ...
        >
            <input
                ...
                type="text"
                placeholder={gameManager.gameEnded
                    ? ""
                    : suburbQuery.getPlaceholder()}
                autocomplete="off"
