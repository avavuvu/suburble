    ...
    {#if feedItem.type === "guess"}
        ...
        <div class="grid grid-cols-2 grid-rows-[auto_auto] gap-2 flex-1 w-full ">
            ...
            <div 
                ...
                <span><span class="font-bold">{distanceToTarget.toFixed(2).slice(0,4)}km</span> away </span>
            </div>
            <div 
                style:animation-delay="{animateInTimeMs * 2}ms"
                class="animate-in rounded col-span-2 gray p-1 {feedItem.overlap.type === "phrase" ? feedItem.overlap.color : ""}">
                {#if feedItem.overlap.type === "phrase"}
                    <div class="p-1">
                        {feedItem.guess.suburb.name} {feedItem.overlap.phrase}
                    </div>
                {:else}
                    <TrainLineDisplay trainLines={feedItem.overlap.trainLines}/>
                {/if}
            </div>
