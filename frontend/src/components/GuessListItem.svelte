<script lang="ts">
    ...
    const getDistanceTextColor = (distance: number) => {
        if(distance === 0) {
    ...
    const getSuburbTextColor = (isCorrect: boolean) => {
        if(isCorrect) {
            return "var(--color-white)"
        }
...
<li class="rounded flex gap-2 bg-gray-300 p-2">
    ...
    <div class="grid grid-cols-2 grid-rows-[auto_auto] gap-2 flex-1 w-full ">
        <div class="font-bold p-1 rounded bg-incorrect text-center"  
            ...
            style:-olor={getSuburbTextColor(guess.isCorrect)}>
            <span>{guess.suburb.name}</span>
        </div>
        <button 
            onclick={setDistanceText}
            class="block p-1 bg-gray-300 rounded text-center "
            style:color={getDistanceTextColor(guess.distanceToTarget)}
            ...
            <span class="bg-white p-1 rounded">
                <span class="inline-block" style:transform={`rotate(${guess.directionToTarget}deg)`}>
                    {guess.emoji} 
                </span>
            </span>
        </button>
