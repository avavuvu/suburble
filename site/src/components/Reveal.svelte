<script lang="ts">
    ...
    const trainLines = new Map<TrainLineName, GoodnessColor>(
        suburb.trainLines.map((trainLine) => [trainLine, "green"]),
    );
</script>
...
>
    <div class=" p-2 rounded text-center col-span-2">
        ...
        <div>
            {didWin
                ...
                : `Better luck next time!` + bestGuess
    ...
    {#if gameManager.hasImage}
        <div
            class=" p-1 rounded text-center flex-col col-span-2 flex justify-center bg-incorrect"
        >
            <img
            ...
            {#if factSheet.attribution}
                ...
                <p class="text-sm text-gray-600">
                    ...
                    <a
                        class="link"
                        href={attribution[factSheet.attribution.type]}
                    >
    ...
    {#if feedItem.revealData.histogramPromise}
                ...
                {#if data.histogram}
                    <div class="grid grid-cols-6 gap-1 w-full h-full">
                        {#each { length: 6 }, index}
                            ...
                            <div
                                class="flex flex-col justify-end gap-2 text-sm text-center"
                            >
                                {#if data.histogram[adjusted]}
                                    <div
                                        class:green={adjusted === guesses}
                                        class="bg-gray-400 rounded-sm px-1 py-0.5 text-xs text-white transition"
